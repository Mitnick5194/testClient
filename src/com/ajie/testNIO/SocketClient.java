...
public class SocketClient {
	...
	public SocketClient(String ip, int port) {
		...
		try {
			...
			/**
			 * while(!channel.finishConnect()){ System.out.println("尝试连接....");
			 * }
			 */
			// 注册连接事件。
	...
	public void pollSelect() throws Exception {
		...
		while (this.selector.select() > 0) {
			//通过打印时间戳 可以知道client先进入这里 然后通知服务端 服务端相应事件 进入while
	...
	public void connect(SelectionKey selectKey) throws Exception {
		try {
			 ...
			 * 有必要的话，调用线程会阻塞直到连接建立完成,finishConnect()方法接着就会返回true 值。
			 */
			/* 设置成非阻塞 */
			channel.configureBlocking(false);
			/* 给服务端发送信息 */
			channel.write(ByteBuffer.wrap(new String("你好 我是客户端 这是我在连接服务器成功时发送给服务器的信息!").getBytes()));
			/* 注册读事件 */
