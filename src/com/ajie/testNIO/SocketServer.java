...
public class SocketServer {
	...
	public SocketServer(String ip, int port) {
		...
		try {
			 ...
			 * 因此有必要取出对等的ServerSocket并使用它来绑定到一 个端口以开始监听连接
			 */
	...
	public void pollSelect() throws Exception {
		/* (阻塞)轮询选择器,直到有事件 */
		int i = 0;
		while ( (i=this.selector.select()) > 0) {
	...
	public void accept(SelectionKey selectKey) throws Exception {
		...
		try {
			...
			/* 发送信息 */
			sc.write(ByteBuffer.wrap(new String("你好 我是服务器 这是我发送给客户端的信息").getBytes()));
			/* 注册读事件 */
