...
public class BlockingQueueTest {
	public static void main(String[] args) throws InterruptedException {
		...
		//增加
		queue.add("a");  //往队列添加元素 如果队列已满 则会排除异常
		queue.put("b"); //如果队列已满 则会一直阻塞等待队列直到队列为非满状态 然后插入(与offer对应)
		queue.offer("c"); //若果队列已满 直接返回false 队列未满 则直接插入病返回true;（与pool对应 用得比较频繁）
		...
		String s = queue.remove(); //删除头元素并返回
